...
import numpy.matlib
 ...
 #changeable, i.e. 'euclidean','cityblock','sqeuclidean','cosine'.
 #changeable, any positive integer. The larger granularity is, the more details the partitioning result gives.

def OfflineADP(data, granularity=11, distancetype='chebyshev'):
    ...
    pos=numpy.zeros(L, dtype = int)
    pos[0]=numpy.argmax(unidata_Gdensity)
    seq=numpy.array(range(0,L))
...
class ADP():
    ...
    def __init__(self, granularity=11, distancetype='chebyshev'):
    ...
    def fit(self, X, y):
        self.pivots, self.cluster_assignments = OfflineADP(X, self.granularity, self.distancetype)
        
        clusters  = numpy.unique(self.cluster_assignments)
        clusters = clusters[clusters>=0]
        self.majority_classes = numpy.empty(len(clusters)).astype(int)
        for i in range(len(clusters)):
            mask = [True if assignment == clusters[i] else False for assignment in self.cluster_assignments]
            labels, counts = numpy.unique(y[mask], return_counts=True, axis=None)
            self.majority_classes[i] = labels[numpy.argmax(counts)]
        
    def predict(self, X):
        distance_matrix = scipy.spatial.distance_matrix(X,self.pivots)
        return numpy.argmin(distance_matrix,axis=1)
