...
class CASTLETabularExplainer(object):
    ...
    def explain_instance(self,
        ...
        if verbose:
            print("[LIMETabularExplainer] - explain_instance. Vicini creati e scalati. L'istanza originale scalata: ", data[0])        
    ...
    def generate_pivots(self, dataset, cluster_assignments, majority_classes):
        ...
        for i in range(int(n_classes)):
            ...
            pivots_per_cluster[clusters[mask]] = np.round(counts[mask]/np.sum(counts[mask])*(num_pivots/(n_classes))).astype(int)
        ...
        for i in range(len(pivots_per_cluster)):
            ...
            if pivots_per_cluster[i] != 0:
                model = KMeans(n_clusters=pivots_per_cluster[i], init='k-means++', max_iter=300, n_init=10, random_state=0).fit(dataset[cluster_assignments == i])
                pivots.append(model.cluster_centers_)
